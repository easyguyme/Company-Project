...
use backend\modules\resque\components\lib\resque\Resque_Job;
use backend\modules\resque\components\lib\Resque\Job\Resque_Job_Status;
use backend\modules\resque\components\ResqueUtil;
use backend\modules\resque\components\lib\Resque\Job\Resque_Job_DirtyExitException;
...
class Resque_Worker
{
    ...
    public static function find($workerId)
    {
        ...
        list ( $hostname, $pid, $queues, $program ) = explode ( ':', $workerId, 4 );
        $queues = explode ( ',', $queues );
        $worker = new self ( $queues, $program );
     ...
     * on.
     * The list of queues should be supplied in the priority that they should
     ...
     */
    public function __construct($queues, $program = '')
    {
        if (! is_array ( $queues )) {
            $queues = array ($queues);
        }
        ...
        empty ( $program ) && ($program = implode ( ',', $this->queues ));
        $this->id = $this->hostname . ':' . getmypid () . ':' . implode ( ',', $this->queues ) . ':' . $program;
    }
     ...
     *
     * @return object boolean of Resque_Job if a job is found, false if not.
     */
     ...
     * The job it is processing will not
     * be completed.
     */
    ...
    public function registerLogger($logger = null)
    {
        ...
        Resque::redis ()->hset ( 'workerLogger', ( string ) $this, json_encode ( array (
            $logger->handler,
            $logger->target
        ) ) );
    }
