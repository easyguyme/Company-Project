...
class ExportUserPlayCount
{
    public function perform()
    {
        $args = $this->args;
        if (empty($args['accountId']) || empty($args['key']) || empty($args['header'])) {
            ResqueUtil::log(['status' => 'fail to export numbers of Users play slotgame', 'message' => 'missing params', 'args' => $args]);
            return false;
        }
        ...
        $rows = ActivityUser::preProcessUserPlayCountData($condition);
        ...
        if ($hashKey) {
            ...
            ResqueUtil::log(['status' => 'fail to export numbers of Users play slotgame', 'message' => 'fail to setQiniuKey', 'filePath' => $filePath]);
            return false;
        }
